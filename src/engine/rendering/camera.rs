...
impl Camera
{
    ...
    pub fn update(&mut self, delta_time: f32, input: &InputState)
    {
        ...
        if input.get_keybind_is_pressed(KeyBind::new("DbgForward")) { self.position += forward * self.speed * delta_time; }
        if input.get_keybind_is_pressed(KeyBind::new("DbgBackward")) { self.position -= forward * self.speed * delta_time; }
        if input.get_keybind_is_pressed(KeyBind::new("DbgLeft")) { self.position -= right * self.speed * delta_time; }
        if input.get_keybind_is_pressed(KeyBind::new("DbgRight")) { self.position += right * self.speed * delta_time; }
        if input.get_keybind_is_pressed(KeyBind::new("DbgUp")) { self.position += up * self.speed * delta_time; }
        if input.get_keybind_is_pressed(KeyBind::new("DbgDown")) { self.position -= up * self.speed * delta_time; }
